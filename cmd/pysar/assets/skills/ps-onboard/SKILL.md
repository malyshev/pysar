...
argument-hint: ""
allowed-tools: Skill mcp__pysar__check_onboarding_status mcp__pysar__save_voice_profile mcp__pysar__save_voice_template mcp__pysar__list_voice_templates mcp__pysar__save_style_profile mcp__pysar__save_style_template mcp__pysar__list_style_templates
---
...
This command is an orchestrator, not a third onboarding conversation of its own. It contains no hardcoded knowledge of what "voice" or "style" specifically involve -- that knowledge lives entirely in `/ps-voice` and `/ps-style`'s own skill files, which this command invokes via the Skill tool and follows exactly as written.

**Always invoke the target pass via the Skill tool, never via `Read` on its `SKILL.md` file.** `Skill(ps-voice)`/`Skill(ps-style)` are pre-approved by `pysar init`'s shipped `settings.json` (portable, name-based permission rules); a raw `Read` on `~/.claude/skills/<pass>/SKILL.md` is gated by a path-based permission that can never be pre-approved the same way (the path is absolute and machine-specific), so it would prompt the author every time for no reason.
...
- **One or more outstanding:** for each outstanding pass, in the order `check_onboarding_status` returned them:
  1. Invoke that pass via the Skill tool, using the pass name from `check_onboarding_status` (e.g. `ps-voice`) as the skill name directly — it is exactly the skill's registered name. Do not `Read` its `SKILL.md` file yourself.
  2. Conduct its entire conversation exactly as that skill defines it — same steps, same questions, same tools, same rules. Do not summarize, thin out, or shortcut it just because it's running from inside `/ps-onboard`; running it here must produce the exact same real result (a validated, saved profile) as the author typing that command directly.
...
- Do not invent your own version of `/ps-voice` or `/ps-style`'s questions from memory instead of invoking their actual current skill via the Skill tool — they can change independently of this file.
- Do not `Read` a pass's `SKILL.md` file directly — always go through the Skill tool, which is what `pysar init`'s settings actually pre-approve.
