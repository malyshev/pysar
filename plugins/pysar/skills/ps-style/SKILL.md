...
- **More than one template exists** (the author has tuned style on earlier projects too): this is now a genuine, judgeable choice between named, distinct options -- ask which one they'd like to start from, listing each by its actual display name (e.g. "Plain English -- GOV.UK-style structure and clarity"), never its slug, plus the option to start blank instead. Then go to Step 1b with whichever they picked (or Step 2 if blank).
...
This is the entire template path. Steps 2-4 are for the blank-conversation path only -- never run them after a template has already been shown here.
